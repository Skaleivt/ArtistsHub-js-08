import{a as E}from"./assets/vendor-BvLu_gPC.js";(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))a(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const r of e.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function o(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function a(t){if(t.ep)return;t.ep=!0;const e=o(t);fetch(t.href,e)}})();const u=document.getElementById("artist-modal"),L=document.getElementById("artist-modal-close"),k=document.getElementById("modal-loader");function _(){k.classList.remove("hidden")}function j(){k.classList.add("hidden")}function w(s){s.target===u&&p()}function C(s){s.key==="Escape"&&p()}function D(){u.classList.add("is-open"),document.body.style.overflow="hidden",L.addEventListener("click",p),u.addEventListener("click",w),document.addEventListener("keydown",C)}function p(){u.classList.remove("is-open"),document.body.style.overflow="",L.removeEventListener("click",p),u.removeEventListener("click",w),document.removeEventListener("keydown",C)}async function F(s){_();try{const n=await s;document.getElementById("artist-name").textContent=n.strArtist||"Unknown",document.getElementById("artist-years").textContent=n.intFormedYear?`${n.intFormedYear}–${n.intDiedYear||"present"}`:"information missing",document.getElementById("artist-sex").textContent=n.strGender||"N/A",document.getElementById("artist-members").textContent=n.intMembers||"N/A",document.getElementById("artist-country").textContent=n.strCountry||"N/A",document.getElementById("artist-bio").textContent=n.strBiographyEN||"No biography available.",document.getElementById("artist-image").src=n.strArtistThumb||"./images/placeholder.jpg";const o=document.getElementById("artist-genres");if(o.innerHTML="",Array.isArray(n.genres)&&n.genres.length>0)n.genres.forEach(t=>{const e=document.createElement("span");e.className="genre-item",e.textContent=t,o.appendChild(e)});else{const t=document.createElement("span");t.className="genre-item",t.textContent="Unknown genre",o.appendChild(t)}const a=document.getElementById("albums-container");if(a.innerHTML="",Array.isArray(n.tracksList)&&n.tracksList.length>0){const t={};n.tracksList.forEach(e=>{const r=e.strAlbum||"Unknown Album";t[r]||(t[r]=[]),t[r].push(e)}),Object.entries(t).forEach(([e,r])=>{const i=document.createElement("div");i.className="album";const d=document.createElement("h2");d.className="album-title",d.textContent=e;const l=document.createElement("div");l.className="track-list-container";const f=document.createElement("div");f.className="track-header",f.innerHTML=`
      <span>Track</span>
      <span>Time</span>
      <span>Link</span>
    `;const g=document.createElement("ul");g.className="track-list",r.forEach(m=>{const S=m.strTrackName||m.strTrack||"Unknown track",y=m.intDuration;function x(T){const b=Math.floor(Number(T)/1e3),q=Math.floor(b/60),U=b%60;return`${q}:${U.toString().padStart(2,"0")}`}const I=y?x(y):"Unknown length",v=m.movie,h=document.createElement("li");h.className="track-item",h.innerHTML=`
        <span class="track-name">${S}</span>
        <span class="track-time">${I}</span>
      ...
      `,g.appendChild(h)}),l.appendChild(f),l.appendChild(g),i.appendChild(d),i.appendChild(l),a.appendChild(i)})}else{const t=document.createElement("p");t.className="no-albums",t.textContent="No tracks available.",a.appendChild(t)}D()}catch(n){console.error("Error loading artist:",n)}finally{j()}}const A="https://sound-wave.b.goit.study/api";async function O(s=1,n=8){try{return(await E.get(`${A}/artists`,{params:{page:s,limit:n}})).data}catch{return iziToast.error({message:"Sorry, there is no more artists. Please try later!",position:"topRight",messageColor:"#fff",messageSize:"16px",backgroundColor:" #EF4040",timeout:3e3}),{}}}async function P(){try{return(await E.get(`${A}/feedbacks`)).data.data}catch(s){return console.error("API Error:",s),{}}}document.querySelector("#artists");const M=document.querySelector(".artists-list"),B=document.querySelector("#load-more-btn");document.querySelector("#no-more-artists-msg");let $=1;const H=8;async function N(){try{const s=await O($,H);if(s.artists.length===0){B.style.display="none",iziToast.info({title:"Notice",message:"No more artists to load",position:"bottomRight",messageColor:"#fff",messageSize:"16px",backgroundColor:" #EF4040",timeout:3e3});return}R(s.artists)}catch(s){console.error("Failed to load artists:",s)}}function R(s){const o=s.filter(a=>a._id).map(a=>{var e;const t=(e=a.genres)!=null&&e.length?a.genres.map(r=>`<span class="artist-card-genre">${r}</span>`).join(""):'<span class="artist-card-genre">Unknown</span>';return`
...
<button type="button" class="artist-card-btn" data-artist-open data-artist-id="${a._id}">
  ...
  <svg class="artist-card-btn-icon" width="8" height="14">
    <use href="#icon-triangle-white"></use>
      ...
      `}).join("");M.insertAdjacentHTML("beforeend",o)}function z(){M.addEventListener("click",s=>{const n=s.target.closest("[data-artist-open]");if(!n)return;const o=n.dataset.artistId;if(!o){console.warn("Artist ID is missing — modal will not open.");return}const a=fetch(`https://sound-wave.b.goit.study/api/artists/${o}`).then(t=>t.json());F(a)})}B.addEventListener("click",async()=>{$+=1,await N()});N();z();async function Y(){const s=await P(),n=document.querySelector("#feedback-list");n.innerHTML="";const o=s.length,a=Math.floor(o/2);s.forEach(e=>{const r=document.createElement("div");r.classList.add("swiper-slide"),r.innerHTML=`
